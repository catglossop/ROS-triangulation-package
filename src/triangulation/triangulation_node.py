...
class transforms:
    ...
    def get_odom_tf(odom, Toi):
        ...
        return True
    def get_odom_tf(odom, init_x, init_y, init_z, init_pos_set, init_pos_set, zero_odom, Toi):
        if zero_odom:
            if !(init_pos_set):
    ...
    def get_transform(tfBuffer, src_frame, tgt_frame):
        got_tf = false
        timeout = 0.025
        tf_temp
...
class TriangulationNode:
    ...
    def initialize_transforms():
        tfBuffer = transforms.tfBufferPtr
...
def callback(img, odom):
    ...
    if collect_image == 1:
        ...
        transforms.get_odom_tf(odom, Toi)
